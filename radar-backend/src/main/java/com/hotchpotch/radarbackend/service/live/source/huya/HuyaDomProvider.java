...
/**
 * 虎牙直播页面 DOM 备用数据源 Provider。
 */
@Component
public class HuyaDomProvider {
    ...
    public LiveSourceResult resolve(ResolvedLiveRoom room) {
        ...
        try {
            HuyaHttpResponse response = webClient.get()
                    ...
                    .block(timeout());
            if (response == null) {
                return LiveSourceResult.temporarilyUnavailable("虎牙 DOM 备用数据源未返回响应");
            }
            ...
            if (response.getStatusCode() >= 400) {
                return LiveSourceResult.temporarilyUnavailable("虎牙 DOM 备用数据源 HTTP 请求失败");
            }
            ...
            return LiveSourceResult.temporarilyUnavailable("虎牙 DOM 备用数据源网络请求失败");
        } catch (RuntimeException exception) {
            return LiveSourceResult.temporarilyUnavailable("虎牙 DOM 备用数据源请求失败");
        }
    }
    ...
    public List<LiveSourceResult> queryStatus(List<LiveAnchor> anchors) {
        ...
        for (LiveAnchor anchor : anchors) {
            if (anchor == null || isBlank(anchor.getRoomId())) {
                results.add(LiveSourceResult.unknown("虎牙 DOM 备用监控对象无效"));
                continue;
            }
