...
/**
 * 斗鱼直播间网页备用数据源 Provider。
 */
@Component
public class DouyuWebProvider {
     ...
     * 斗鱼网页解析器。
     */
    private final DouyuWebParser parser;
    ...
    public LiveSourceResult resolve(ResolvedLiveRoom room) {
        if (room == null || isBlank(room.getRoomId())) {
            return LiveSourceResult.unknown("斗鱼网页房间标识为空");
        }

        String url = buildUrl(properties.getWebBaseUrl(), "/" + room.getRoomId());
        try {
            ...
            return LiveSourceResult.temporarilyUnavailable("斗鱼直播间网页 HTTP 请求失败");
        } catch (WebClientRequestException exception) {
            return LiveSourceResult.temporarilyUnavailable("斗鱼直播间网页网络请求失败");
        } catch (RuntimeException exception) {
            return LiveSourceResult.temporarilyUnavailable("斗鱼直播间网页请求失败");
        }
    }
    ...
    public List<LiveSourceResult> queryStatus(List<LiveAnchor> anchors) {
        ...
        for (LiveAnchor anchor : anchors) {
            if (anchor == null || isBlank(anchor.getRoomId())) {
                results.add(LiveSourceResult.unknown("斗鱼网页监控对象无效"));
                continue;
            }
    ...
    /**
     * 将网页快照与已有资料合并，避免网页暂缺字段时覆盖最后一次有效资料。
     *
     * @param result 数据源结果
     * @param anchor 已保存主播
     * @return 合并后的结果
