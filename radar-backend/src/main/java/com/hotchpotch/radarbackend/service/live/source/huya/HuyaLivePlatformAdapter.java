 ...
 */
@Component
public class HuyaLivePlatformAdapter implements LivePlatformAdapter {

    /**
     * 虎牙页面主数据源 Provider。
     */
    private final HuyaPageProvider pageProvider;

    /**
     * 虎牙 DOM 备用数据源 Provider。
     */
    private final HuyaDomProvider domProvider;

    /**
     * 创建虎牙平台适配器。
     *
     * @param pageProvider 虎牙页面主数据源 Provider
     * @param domProvider 虎牙 DOM 备用数据源 Provider
     */
    public HuyaLivePlatformAdapter(
            HuyaPageProvider pageProvider,
            HuyaDomProvider domProvider) {
    ...
    public boolean supports(LivePlatform platform) {
        return LivePlatform.HUYA == platform;
    }

    @Override
    public LiveSourceResult resolve(ResolvedLiveRoom room) {
        LiveSourceResult primaryResult = pageProvider.resolve(room);
        if (primaryResult.isAvailable()) {
            return primaryResult;
        }
        return routeFallback(primaryResult, domProvider.resolve(room));
    }
    ...
    public List<LiveSourceResult> queryStatus(List<LiveAnchor> anchors) {
        ...
        List<LiveSourceResult> primaryResults = pageProvider.queryStatus(anchors);
        ...
        for (int index = 0; index < anchors.size(); index++) {
            LiveSourceResult primaryResult = index < primaryResults.size()
                    ...
                    : LiveSourceResult.unknown("虎牙主数据源返回结果数量不足");
            if (primaryResult.isAvailable()) {
            ...
            if (anchor == null) {
                results.add(LiveSourceResult.unknown("虎牙监控对象为空"));
                continue;
            }
            ResolvedLiveRoom room = new ResolvedLiveRoom(
                    ...
                    anchor.getRoomId(),
                    anchor.getRoomUrl());
            results.add(routeFallback(primaryResult, domProvider.resolve(room)));
        }
        return results;
    }
    ...
    private LiveSourceResult routeFallback(
        ...
        if (backupResult == null) {
            return LiveSourceResult.unknown("虎牙 DOM 备用数据源未返回结果");
        }
        ...
        if ((primaryResult != null
            ...
            return LiveSourceResult.temporarilyUnavailable("虎牙主备数据源均无法可靠返回结果");
        }
        return LiveSourceResult.unknown("虎牙主备数据源结果无法确认");
    }
}
